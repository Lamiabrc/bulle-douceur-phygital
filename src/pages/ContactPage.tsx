...
const ContactPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData._honeypot && formData._honeypot.trim().length > 0) {
      return;
    }

    setIsSubmitting(true);
    try {
      // 1) Enregistrer le lead (n'échoue pas l'envoi d'email si ça plante)
      ...
      // 2) Envoyer l’email vers la boîte Outlook via FormSubmit (obligatoire)
      const emailPayload = {
        name: formData.nom,
        email: formData.email,
      ...
      // Si l'email échoue, on considère l'envoi KO
      if (
        leadRes.status === "rejected" ||
        (emailRes.status === "fulfilled" ? !emailRes.value.ok : true)
      ) {
        throw new Error(
          emailRes.status === "fulfilled" ? `Email HTTP ${emailRes.value.status}` : "Email send failed"
        );
      }
              ...
              <div>
                ...
                <div className="bg-primary/5 p-4 rounded-lg">
                  <p className="text-sm text-primary font-semibold">
                    ✓ Toutes nos prestations sont sur mesure
                    <br />
